(() => {
    ...
    function EffectCards(_ref) {
        ...
        const setTranslate = () => {
            ...
            for (let i = 0; i < slides.length; i += 1) {
                ...
                if (progress < 0) tX = `calc(${tX}px + (${tXAdd * Math.abs(4 * progress)}%))`; else if (progress > 0) tX = `calc(${tX}px + (-${tXAdd * Math.abs(progress)}%))`; else tX = `${tX}px`;
    ...
    da.init();
    function formBar() {
        const peopleCompleted = 40;
        const maxPeople = 100;
        const bar = document.querySelector(".block-footer__progress-bar");
        bar.style.width = peopleCompleted / maxPeople * 100 + "%";
